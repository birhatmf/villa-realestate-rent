  ...
  return (
    <a
      ...
      className={`group flex items-center gap-0 overflow-hidden rounded-full bg-ink px-4 py-3.5 text-canvas shadow-[0_10px_30px_-8px_rgba(0,0,0,0.45)] transition-all duration-500 hover:bg-[#25D366] hover:text-white ${
        floating
          ? `fixed z-40 ${config?.position === 'left' ? 'left-6' : 'right-6'}`
          : 'relative'
      }`}
      // Sayfalarda tam genişlik sabit bir alt çubuk varsa (örn. villa detayındaki mobil
      // fiyat çubuğu), o sayfa --wa-bottom-offset'i ayarlar; yoksa varsayılan 1.5rem.
      style={floating ? { bottom: 'var(--wa-bottom-offset, 1.5rem)' } : undefined}
    >
